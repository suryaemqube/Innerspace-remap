...
const SwiperScroll = () => {
		...
		if (swiper) {
			let ctx = gsap.context(() => {
				let intentObserver = ScrollTrigger.observe({
					type: "wheel,touch",
					onUp: () => {
						console.log("up", currentIndex1, animating)
						!animating && gotoPanel(currentIndex1 + 1, true)
					},
					onDown: () => {
						// setCurrentIndex1(currentIndex1 - 1)
						// console.log("down", currentIndex1)
						!animating && gotoPanel(currentIndex1 - 1, false)
					},
				...
				ScrollTrigger.create({
					...
					onEnter: (self) => {
						if (preventScroll.isEnabled === false) {
							...
							gotoPanel(currentIndex1 + 1, true);
						}
					},
					onEnterBack: (self) => {
						if (preventScroll.isEnabled === false) {
							...
							preventScroll.enable();
							intentObserver.enable();
							gotoPanel(currentIndex1 - 1, false);
				...
				console.log("currentIndex1--", currentIndex1, numPanels)


				function gotoPanel(currentIndex1, isScrollingDown) {
					...
					if (
						(currentIndex1 === numPanels && isScrollingDown) ||
						...
						preventScroll.scrollY(
							preventScroll.scrollY() + (currentIndex1 === numPanels ? 1 : -1)
						);
						return;
					}
					swiper.slideTo(currentIndex1);
	...
	return (
		<>
			<div className="brand" style={{ "background": "black", "textAlign": "center" }} >
				<a
					href="https://www.creativecodingclub.com/bundles/creative-coding-club?src=cdp3droll" target="_blank">
					<img
						src="https://assets.codepen.io/32887/logo-ill.svg" width="300" alt="" />
				</a>
			</div>
			<div className="space" style={{ "padding": "20px", "background": "#efefef" }}>
				...
				<ul>
					<li>
						If you get to slide 6 and scroll all the way down, sometimes on the way up you can bypass the pinning and navigation through the Swiper gallery.
					</li>
			...
			<div className="gallery" ref={galleryRef}>
				...
				<div id="currentSlide"
					style={{ fontSize: "4vh", fontWeight: "bold", fontFamily: "sans-serif", width: "8em", padding: "20px", background: "#ccc", borderRadius: "2.5em", textAlign: "center", margin: "20px auto auto auto" }}>
					1</div>
			...
			<div className="space" style={{ padding: "20px", background: "#efefef" }}>
				...
				<p>Even though this seems like a neat UI treatment, it is very strange to lock the scrollbar in place. I really
					don't think the user should be prevented from using a scrollbar. I tried hiding the scrollbar by setting
					<span style={{ fontFamily: "Courier, serif", fontWeight: "bold", }}>visibility:hidden</span> on body when scrolling
					should
					be disabled but that was weird too. </p>

				<p>I would suggest the following 3 alternatives</p>
				<ol>
					<li>Do <strong>not pin</strong> the Swiper section. Provide a clear UI for horizontal navigation with
						prev/next
						buttons and make it clear that swiping is allowed too.</li>
					<li>Ditch Swiper. Pin the section and control it with a <a
						href="https://greensock.com/3-8/#containerAnimation" target="_blank">container animation</a>. Using
						this technique the scroll-wheel and scrollbar can be used
						consistently with no lockups or funny business.</li>
					<li>Create a full-screen experience where the <strong>scrollbar is always hidden</strong> and the only way
						to
						navigate is via scrollwheel or touch gestures. </li>

				</ol>

			</div>
			<div className="brand" style={{ background: "black", textAlign: "center" }}><a
				href="https://www.creativecodingclub.com/bundles/creative-coding-club?src=cdp3droll" target="_blank"><img
					src="https://assets.codepen.io/32887/logo-ill.svg" width="300" alt="" /></a>
			</div>
		</>
	)
}
